...
We implemented complete ARM bare-metal code with vector table, reset handler, and system initialization. The quantized model compiles successfully (31.7 KB flash, 10.5 KB RAM) and fits within STM32H7 constraints with 98\%+ headroom. Based on ARM Cortex-M7 specifications, we estimate 42 ms inference latency—suitable for real-time edge applications at $\sim$24 FPS.

Renode simulation validated functional correctness. Physical hardware deployment remains future work but resource analysis and cross-compilation success provide strong evidence of deployability.
...
\item \textbf{Physical Hardware}: Deploy on actual STM32H7 development board to validate theoretical performance estimates with measured latency
\item \textbf{Additional Platforms}: Extend validation to Raspberry Pi, Jetson Nano, and RISC-V targets
