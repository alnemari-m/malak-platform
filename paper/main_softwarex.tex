...
\author{Mohammed Alnemari$^{1,3}$, Rizwan Qureshi$^{4}$, Nader Begrazadah$^{3}$\\[4pt]
\small $^{1}$Faculty of Computer Science and Information Technology, University of Tabuk, Tabuk, Saudi Arabia\\
\small $^{3}$AIST Research Center, University of Tabuk, Tabuk, Saudi Arabia\\
\small $^{4}$University of California, Irvine, Irvine, California, USA\\[2pt]
\small \texttt{mohammedalnemari@gmail.com}}
...
\end{tikzpicture}
\caption{Malak software architecture. Solid arrows show data flow between modules. Dashed arrows indicate CLI access points. Modules are grouped into three layers: model preparation, deployment and analysis, and the command-line interface.}
